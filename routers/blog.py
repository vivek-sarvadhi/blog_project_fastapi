...
def create(title: str = Form(...), body: str = Form(...), db: Session = Depends(database.get_db)):
...
def destory(id:int , db: Session = Depends(database.get_db)):
...
def update(id:int, request: blog_schema.Blog, db: Session = Depends(database.get_db)):
    blog = db.query(models.Blog).filter(models.Blog.id == id)
    if not blog.first():
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,detail=f"Blog with the id {id} is not available")
    # blog.update(request)
